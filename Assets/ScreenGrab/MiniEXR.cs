...
namespace MiniEXR {
	...
	public static class MiniEXR {
		...
		public static void MiniEXRWriteRGBA (string _filePath, uint _width, uint _height, Color[] _colorArray) {
			Debug.Log("check");
			System.IO.File.WriteAllBytes(_filePath, MiniEXRWriteRGBA(_width, _height, _colorArray) );
		}
		...
		public static byte[] MiniEXRWriteRGBA (uint _width, uint _height, Color[] _colorArray) {
			Debug.Log("check2");
			float[] rgbaArray = new float[ _colorArray.Length * 4 ];
 
			for (int i = 0; i < _colorArray.Length; i++) {
				rgbaArray[i * 4] = _colorArray[i].r;
				rgbaArray[i * 4 + 1] = _colorArray[i].g;
				rgbaArray[i * 4 + 2] = _colorArray[i].b;
				rgbaArray[i * 4 + 3] = _colorArray[i].a;
			}
			Debug.Log(rgbaArray[0]+" "+rgbaArray[1]+" "+rgbaArray[2]+" "+rgbaArray[3]);
			return MiniEXRWrite(_width, _height, 4, rgbaArray);
		}
